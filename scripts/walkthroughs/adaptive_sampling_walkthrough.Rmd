...
For any adaptive sampling programme, we first need some initial data on which we can base our further sampling. Here, we use the simulated distribution of the hipporhinostricow, latin name _Spikus milligani_. This species is located across the very edge of Western Europe and is thought to be slowly increasing its range northwards and eastwards. It's thought to be very reliant on areas with high range in rainfall and warmer temperatures. 
...
We can extract model uncertainty across GB by predicting the standard error back onto the map of GB. Here we plot the predicted probability of presence (figure 4) and the standard error of the model (figure 5).
...
To do this, we will select a random location from all of the places that haven't already been visited (i.e. the empricial criterion). We will then remove all other unvisited locations within `r print(dist_apart)`m of it, and sample another point. This will be done until `r print(nlocs)` have been sampled (figure 6).
