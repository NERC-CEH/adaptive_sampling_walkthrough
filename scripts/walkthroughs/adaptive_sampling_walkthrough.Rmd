...
# plot
plot(st_geometry(uncertrast), pch = 19)
plot(uncertgrid, add = TRUE)
plot(uncertrast[1,], col = "red", pch = 19, add = TRUE)
...
# for(i in 1:1000){
  ...
  highest_uncert <- uncertsftoedit[i,]
  
  # store that point
  uncertoutdf <- rbind(uncertoutdf, highest_uncert)
  
  # find the grid cell it's in or if it touches one or more than one
  ...
  highgridcellind <- c(unlist(highgridcellind), unlist(highgridcellind_touch))
  
  highgrid <- uncertgrid[c(highgridcellind)]
  
  # find all the points within that grid cell
  points_contained <- unlist(st_contains(highgrid, uncertsf))
...
plot(uncertgrid, add = TRUE)
plot(outdf, col = "red", pch = 19, cex= 0.5, 
     add = TRUE)
### has it worked?!?!
