...
gdf <- st_make_grid(samp_sf$geometry, cellsize = dist_apart, square = FALSE)
...
final_sample_ind <- sample(1:nrow(outdf), size=nlocs)
...
Select a random location and then remove all points within X distance of it.
...
# plot
plot(empabsr)
plot(empabsoutdf, add = TRUE, pch = 19, cex = 0.5)
...
Select the location with the highest uncertainty and then remove all points within X distance of it.
