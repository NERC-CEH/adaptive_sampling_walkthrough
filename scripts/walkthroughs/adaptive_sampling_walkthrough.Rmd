...
## optimise sampling for 100 locations
batch <- optimise_sampling_uncertainty(uncertainty_dataframe = uncertsf,
                                       number_locactions = nlocs/4)
head(batch)

plot(species_distrib_rast)

## check whether they find new records
new_locs <- check_distrib(new_records = terra::vect(batch), true_species_distrib = species_distrib_rast) 

head(species_sampled_distrib_df)
