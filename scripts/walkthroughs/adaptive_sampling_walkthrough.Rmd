...
# plot
ggplot(species_sampled_distrib_df, aes(x,y, fill = factor(observations))) +
  geom_tile() +
...
no_records <- species_sampled_distrib_df[species_sampled_distrib_df$observations==0,]
...
# plot
ggplot() +
  geom_tile(data = species_sampled_distrib_df, aes(x,y, fill = factor(observations))) +
  scale_fill_manual(name = '', labels = c("Absences", "Observations"),
                    values = c("#E69F00", "#009E73")) +
  geom_sf(data = empabsoutdf, size = 0.5, aes(colour = "New recording\nlocations")) +
...
ggplot() +
  ...
  scale_fill_viridis_c(name = "Standard error") +
  geom_sf(data = uncertoutdf, size = 0.5, aes(colour = "New recording\nlocations")) +
...
top_uncert <- sorted_data[1:nlocs, c("x", "y")]
