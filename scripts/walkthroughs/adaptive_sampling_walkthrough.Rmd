...
The first step in the adaptive sampling process is to define a criterion layer, which is a layer that contains our sampling priorities. It is from this layer that we choose locations at which to carry out further sampling. This can broadly be done in two ways, using Empirical methods or model-based methods. There are many ways in which this can be done but we demonstrate only one of each. See the paper for more examples.
