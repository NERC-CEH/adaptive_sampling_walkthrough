...
# model_rounds_batches <- function(modelbased_crit_sf, 
...
all_uncert_data <- rbind(initial_uncert_data, modeluncertainty_out)
...
new_locs_out$batch <- factor(new_locs_out$batch)
all_uncert_data$batch <- factor(all_uncert_data$batch, levels = c("initial", 1, 2, 3))

levels(new_locs_out$batch) <- c("Batch 1","Batch 2",
                                "Batch 3","Batch 4")
...
model_optim <- ggplot() +
  ...
  scale_fill_viridis_c(name = "Standard error", begin = 0.1) +
  geom_point(data = new_locs_out, aes(x,y), size = 0.8) +#,colour = factor(observations))) +
  coord_sf(expand = FALSE) +
  # ylim(0, 350000) +
  ylim(622500, 980000) +
  xlim(45000, 425000) +
...
new_records_plot <- 
  ggplot() +
  geom_sf(data = gboutline) +
  geom_point(data = new_locs_out[new_locs_out$observations==1,], aes(x,y), colour="darkgreen") +
  coord_sf() +
  theme_void()
new_records_plot
...
#### the easy way of showwing batches
...
ggplot() +
  # geom_raster(data = modeluncertainty_out, aes(x, y, fill = se.fit)) +
  # scale_fill_viridis_c(name = "Standard error", begin = 0.1) +
  geom_point(data = new_locs_out[new_locs_out$observations==1,], aes(x,y)) +
  coord_sf(expand = FALSE) +
