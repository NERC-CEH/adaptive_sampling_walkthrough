...
For any adaptive sampling programme, we first need some initial data on which we can base our further sampling. Here, we use the simulated distribution of the hipporhinostricow, latin name _Spikus milligani_. This species is located across the very edge of Western Europe and is thought to be slowly increasing its range northwards and eastwards. It's thought to be very reliant on areas with a high range in rainfall and warmer temperatures. 

The code to simulate this species' distribution is located in `/scripts/processing/simulate_species.R`. We then sample this true species distribution probabibalistically using the sum of GB's urban and suburban distribution. We did this to mimic the real life situation where people tend to go wildlife recording close to their homes. This process has generated the distribution of the hipporhinostricow (Figure 1).
...
The first step in the adaptive sampling process is to define a criterion layer, which is a layer that contains our sampling priorities. It is from this layer that we choose locations at which to carry out further sampling. This can broadly be done in two ways, using empirical methods or model-based methods. There are many ways in which this can be done but we demonstrate only one of each here. See the paper for more examples.

### Step 1a. Empirical criterion

An empirical criterion involves using only the data to determine locations for further sampling. Here, we identify regions that have not yet been sampled. In the code below, we convert our species records into a 1km binary raster format for presences (`1`) and absences (`0`). This creates a raster map of the species' distribution which will be our *empirical sampling layer* (figure 2).
...
obssp_dsitrib +
  labs(caption = "Figure 2 The empirical sampling layer containing information\nabout hipporhinostricow presences and absences across GB") +
  theme(text = element_text(size = 18),
        plot.caption.position = "plot")
...
Before running our models, we first need to determine absences. In many SDM studies we have only presence data, and so the choice of method to get pseudoabsences is important. For simplicity, we choose to use a random background approach as we are not interested in its effect on model performance. We will randomly sample the same number of points as there were presences from all of the locations which do not already have records (figure 3).
...
We fit a binomial GAM using these presences and pseudoabsences. We model this with four explanatory variables fitted as splines: the percentage of improved grassland, annual mean temperature, annual precipitation and a spatial spline. From this model we can then plot the prediction and the standard error back onto a map of GB. The standard error, as a measure of model uncertainty, is the model-based sampling metric that we will use to determine subsequent sampling. Targeting regions of high model uncertainty could be valuable for improving model performance.
...
We can extract model uncertainty across GB by predicting the standard error back onto the map of GB. Here we plot the predicted probability of presence (figure 4) and the standard error of the model (figure 5). The standard error will be our model-based uncertainty criterion which we will use to determine future sampling locations.
...
Once a criterion has been decided, we can then use it to choose new sampling locations. There are many approaches that can be used to do this. Here, we provide examples of two methods which broadly lie at opposite ends of the spectrum of possible approaches and apply these to both our empirical and model-based criterions. 

We have chosen to take `r (nlocs)` new samples; i.e. `r (nlocs)` locations will be visited to determine the presence of the hipporhinostricow. We first use optimised selection methods to sample both our empirical and model-based criterions, and then use the independent draws method. For more information about the merits of each method, see the paper.

### Step 2a. Optimised

#### Empirical method - optimised

First, we will sample our empirical criterion using an optimised approach. As a reminder, our empirical criterion is a spatial layer that identifies all unvisited locations. To sample from this in an optimised way, we select a random location from this criterion. We then remove all other locations within `r (dist_apart)`m of it and sample another point. This is done until we have sampled `r (nlocs)` locations (figure 6).
...
We choose to optimise our sampling of the model-based criterion layer in the same way as for the empirical later. First, we select the location in GB that has the highest model uncertainty. Then, we  remove all other locations within `r (dist_apart)`m of this location, and then sample the location with the next highest uncertainty. We repeat this until `r (nlocs)` have been sampled.
...
The alternative to optmising our choice of sampling locations is to select each location independently from one another.

We do this for both the empirical and model-based criterion layers.
...
As an example, here we carry out four rounds adaptive sampling, with a batch size of `r (round(nlocs/nrounds))`. The function below runs the whole adaptive sampling process using model uncertainty and an optimised sampling method, as outlined above. In each step it samples `r (round(nlocs/nrounds))` locations that are `r (dist_apart)`m apart and reruns the GAM using both the initial and new data. The results of this are showwn in Figure 10.
...
as_rounds <- adaptive_sampling_rounds(
  ...
  # I could add round to this so that we can account for round number in the modelling?
  initial_species_records = species_records,
  
  # distance between records to maintain
  distance_apart = dist_apart,
  
  # true species distribution - for checking whether new samples find a species
...
This adaptive sampling process results in a set of new data (figure 11). 
