...
emp_optim <- ggplot() +
  ...
  scale_colour_manual(name = "", values = "white") +
  ylim(0, 350000) +
...
empindep_draws <- ggplot() +
  ...
  coord_sf(expand = FALSE) +
  scale_colour_manual(values = c("white"), name = "") +
  scale_fill_manual(name = '', labels = c("Absences", "Observations"),
                    values = c("#E69F00", "#009E73")) +
  ylim(0, 350000) +
...
Select the 750 locations with the highest uncertainty
...
In adaptive sampling there will be a decision to make about the number of rounds of adaptive sampling and how many points to sample at each time point. For example, if data collection involved a lot of organisation and a trip overseas, it might be that a single collection incident is logistically possible. In other cases, it might be that we are able to carry out sampling over several field seasons, with the criterion layer recalculated at each step.
...
Here we show how this is possible. Using four batches of 100
...
# output data frames
new_species_observations <- data.frame()
modeluncertainty_out <- data_frame()
...
for(batch_num in 1:nbatches) {
  ...
  if(batch_num>1){
    print(paste("Removing potential sampling occasions within", distance_apart, "m of previous adaptively sampled records"))
    
    # convert new records to sf
    new_species_observations_sf <- st_as_sf(new_species_observations[,c("x", "y")], 
                                            coords = c("x", "y"), 
                                            crs = 27700)
    # find all points within x distance of highest uncert
    ...
    if(length(points_within_dist)>0){
      # remove those points from the uncertainty dataframe 
      modelbased_crit_sf <- modelbased_crit_sf[-points_within_dist,]
    }
...
model_optim <- ggplot() +
  geom_raster(data = modelbased_criterion, aes(x, y, fill = se.fit)) +
  scale_fill_viridis_c(name = "Standard error", begin = 0.1) +
  geom_point(data = new_species_observations, aes(x,y,colour = factor(batch))) +
  coord_sf(expand = FALSE) +
  # scale_colour_manual(name = "", values = c("black", "white", "red", "orange")) +
  ylim(0, 350000) +
...
ggplot(modeluncertainty_out, aes(x,y, fill = se.fit)) +
  geom_tile() +
  ylim(250000, 350000) +
  xlim(300000, 400000) +
  facet_wrap(~batch)
