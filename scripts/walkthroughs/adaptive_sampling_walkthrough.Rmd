...
# the species records to be updated each time
species_observations <- species_records
...
for(batch_num in 1:nbatches) {
  ...
  print(paste("Preparing for modelling"))
  {
    ...
    all_records <- rbind(species_observations[,1:2], new_species_observations[, c("x", "y")])
    
    ## remove duplicates
    print(paste(sum(duplicated(all_records)), "were removed.", 
                round(nlocs/nbatches)-sum(duplicated(all_records)), "remain for modelling."))
...
ggplot() +
  geom_point(data = new_species_observations, aes(x, y)) +
  facet_wrap(~batch)
